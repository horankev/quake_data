...
To get an interpretable measure of faultline concentration in any area, they are transform from linestrings to polygons by setting a buffer  of 10km around them, as shown below. This is now in units of kilometres squared and be compared with the area of administrative units to give concentrations.
...
ggplot() + 
  ...
  geom_sf(data=provinces_df, aes(fill=fault_concentration), linewidth=0.5) +
...
ggarrange(
  ...
  geom_sf(data=quakes_df |> filter(magfact != "S") |> filter(province != "undersea"), 
    ...
    labs(title = "within province / county") + 
  annotation_scale() +
  coord_sf(datum=NA),
...
Including all earthquakes, both undersea and under land, Their counts can be represented as density plots. When faceted by magnitude, we get the following:
